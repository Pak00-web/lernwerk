...
function viewEinstellungen(){
  ...
  app.innerHTML = seitenKopf('Mein Lernen', 'Einstellungen') + `<div class="panel setup einst">
    ...
    <div><div class="eyebrow">Konto</div><div class="row"><button class="btn" data-ziel="#/konto">${konto ? 'Profil & Konto' : 'Anmelden / Konto anlegen'}</button><button class="btn ghost" data-ziel="#/datenschutz">Datenschutz</button><button class="btn ghost" data-ziel="#/abzeichen">${ICON.trophy}Abzeichen</button>${konto && window.LW_SYNC.profil() && window.LW_SYNC.profil().admin ? '<button class="btn" data-ziel="#/admin">Admin-Bereich</button>' : ''}</div></div>
  </div>`;
